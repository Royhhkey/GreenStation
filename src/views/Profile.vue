    ...
    <div class="profile-content">
        ...
        <template #title>
          <div class="info-card-title-container">
            <div class="info-card-title">{{ isEditing ? '编辑个人信息' : '个人信息' }}</div>
            <div>
              <a-button 
                v-if="!isEditing"
                type="primary" 
          ...
          <a-descriptions :column="1" bordered>
            ...
            <a-descriptions-item label="用户名">
              ...
              <a-input v-else v-model:value="editForm.username" size="small" placeholder="请输入用户名" />
            </a-descriptions-item>

            <a-descriptions-item label="学号">
              <span v-if="!isEditing">{{ userInfo.XH || '未设置' }}</span>
              <a-input v-else v-model:value="editForm.XH" size="small" placeholder="请输入学号" />
            </a-descriptions-item>
            
            <a-descriptions-item label="邮箱">
              <span v-if="!isEditing">{{ userInfo.email || '未设置' }}</span>
              <a-input v-else v-model:value="editForm.email" size="small" placeholder="请输入邮箱" />
            </a-descriptions-item>
            
            <a-descriptions-item label="手机号">
              <span v-if="!isEditing">{{ userInfo.phoneNumber || '未设置' }}</span>
              <a-input v-else v-model:value="editForm.phoneNumber" size="small" placeholder="请输入手机号" />
            </a-descriptions-item>
            
            <a-descriptions-item label="寝室">
              <span v-if="!isEditing">{{ userInfo.QSH || '未填写' }}</span>
              <a-input v-else v-model:value="editForm.QSH" size="small" placeholder="请输入寝室" />
            </a-descriptions-item>
            
            <a-descriptions-item label="年级">
              <span v-if="!isEditing">{{ userInfo.grade || '未填写' }}</span>
              <a-input v-else v-model:value="editForm.grade" size="small" placeholder="请输入年级" />
            </a-descriptions-item>
            
            <a-descriptions-item label="专业">
              <span v-if="!isEditing">{{ userInfo.ZY || '未填写' }}</span>
              <a-input v-else v-model:value="editForm.ZY" size="small" placeholder="请输入专业" />
            </a-descriptions-item>
          </a-descriptions>
  </a-card>
      ...
      >
        <div class="avatar-edit-modal">
          ...
          <div class="avatar-actions">
            ...
            <a-button type="primary" @click="handleAvatarSave" :loading="avatarLoading">
              保存
            </a-button>
          </div>
        </div>
...
import {removeEmptyProperties,objectToString,replaceUrlRegex,extractDateFromISO,getChangedValues} from '@/utils'
import {
          getMyproduct,
          ModifyUserInfo,
          AvatarUpload,
...
const handleProductSave = async (productData) => {
  ...
  console.log('myProducts before', myProducts.value);
    ...
    if (productData.category_info && productData.category_info.cid !== undefined && productData.category_info.cid !== null) {
